...
html_title = """
    ...
    <center><h1 class= "title-text">Adidas Interactive Sales Dashboard</h1></center>"""
...
results = df.groupby(by=df["Month_Year"])["TotalSales"].sum().reset_index()

with col5:
    fig1 = px.line(results, x = "Month_Year", y = "TotalSales", title="Total Sales over Time", 
                   template="gridon")
...
with view2:
    ...
    expander.write(data)
with dwn2:
    st.download_button("Get Data", data = results.to_csv().encode("utf-8"),
                       file_name="Monthly Sales.csv", mime="text/csv")
st.divider()

results1 = df.groupby(by="State")[["TotalSales", "UnitsSold"]].sum().reset_index()

# add the units sold as a line chart on a secondary y-axis
fig3 = go.Figure()
fig3.add_trace(go.Bar(x = results1["State"], y = results1["TotalSales"], name="Total Sales"))
fig3.add_trace(go.Scatter(x = results1["State"], y = results1["UnitsSold"],
                           mode = "lines", name = "Units Sold", yaxis="y2"))
fig3.update_layout(
    title = "Total Sales and Unit Sold by State",
...
with view3:
    ...
    expander.write(results1)
with dwn3:
    st.download_button("Get Data", data=results1.to_csv().encode("utf-8"),
...
with col7:
    st.subheader(":point_right: Total Sales by Region and City in a Treemap")
...
with view4:
    results2 = df[["Region", "City", "TotalSales"]].groupby(by=["Region","City"])["TotalSales"].sum()
    expander = st.expander("view data for Total Sales by Region and City ")
    expander.write(results2)
with dwn4:
    st.download_button("Get Data", data=results2.to_csv().encode("utf-8"),
                       file_name="Sales_by_Region", mime="text/csv")

_, view5, dwn5 = st.columns([0.5, 0.45,0.45])
with view5:
    expander = st.expander("Viw Sales Raw Data")
    expander.write(df)
with dwn5:
